...
object Common {
  ...
  private val pDependVersion =
    scala.io.Source.fromFile(".pdepend-version").mkString.trim
      ...
      case cmd @ Cmd("ADD", _) =>
        List(
          ...
          Cmd(
            ...
            s"""
               |apk update &&
               |apk add bash curl php php-tokenizer php-xml php-cli php-pdo php-curl php-json php-phar php-ctype php-openssl php-dom &&
               |curl -O https://static.pdepend.org/php/$pDependVersion/pdepend.phar &&
               |rm -rf /tmp/* &&
